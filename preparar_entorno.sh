...
# 2. BLINDAJE DE ENLAZADOR TOTAL: Escribimos el listado maestro definitivo de xf86drm.h en C puro con el mapa de loader y buses
...
# 4. TU ESTRATEGIA DE NACIMIENTO FORZADA INMUTABLE: Python abre el wsi_common.h original en la raíz del repositorio e inyecta los campos de factoría directamente adentro de las tres estructuras principales antes de compilar
...
# El truco maestro: Metemos la declaración del puntero de función de Vulkan y los miembros de AHardwareBuffer directo arriba de la llave de cierre para evadir cualquier restricción
...
if [ -f "$BASE_PWD/build_drm/libdrm.so" ]; then
  ...
  cp -f "$BASE_PWD/build_drm/libdrm.so" "$NDK_LIB_DIR/32/libdrm.so" 2>/dev/null || true
fi
