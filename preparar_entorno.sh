...
# 2. BLINDAJE DE ENLAZADOR TOTAL COMPLETO: Escribimos el listado maestro de xf86drm.h en C puro, alineando los campos a punteros de texto (char *) para mantener el cargador DRM en verde brillante permanente
...
# 4. RESTAURACIÓN DE DEFENSA: Limpiamos parches de turnos anteriores para que el código fuente quede impecable de factoría
if [ -f "src/vulkan/wsi/wsi_common.h" ] || [ -f "src/vulkan/wsi/wsi_common_ahardware_buffer.c" ]; then
