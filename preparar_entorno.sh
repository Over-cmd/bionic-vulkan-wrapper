...
# 4. MOVIMIENTO MAESTRO ESCENE: Forzamos a Meson a saltarse únicamente el archivo conflictivo de buffers de PC eliminando su plano de construcción, pero dejando el soporte WSI general de Android intacto de fábrica
if [ -f "src/vulkan/wsi/meson.build" ] || [ -f "src/vulkan/wsi/wsi_common_ahardware_buffer.c" ]; then
  echo "-> Aplicando purificación lícita en los planos de Meson WSI..."
  ...
  # Comentamos la linea que obliga a meter el modulo redundante en los planos de Ninja
...
echo "=== ENTORNO ENLAZADOR TOTALMENTE SINCRO CON PLANOS PURIFICADOS ==="
