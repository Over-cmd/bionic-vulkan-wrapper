#!/bin/bash
set -e
echo "=== ETAPA C: COMPILACIÓN DUAL DE ALTO RENDIMIENTO CON EL INTERRUPTOR DE LEEGAO EN ARQUITECTURA DE ESCRITORIO ==="
...
# CORRECCIÓN DE RUTAS DE ENLACE DE 32 BITS: Apuntamos de forma estricta a las carpetas de salida físicas de CMake para SPIRV-Tools
printf "[binaries]\nc = '$NDK_PATH/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi26-clang'\ncpp = '$NDK_PATH/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi26-clang++'\nar = '$NDK_PATH/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-ar'\nstrip = '/bin/true'\npkg-config = '/usr/bin/pkg-config'\n[built-in options]\nc_args = ['--sysroot=$SYSROOT_PATH', '-I$BASE_PWD/spirv_source/include', '-I$BASE_PWD/local_include', '-I$BASE_PWD/local_include/libdrm', '-march=armv7-a', '-mfloat-abi=softfp', '-mfpu=neon']\ncpp_args = ['--sysroot=$SYSROOT_PATH', '-I$BASE_PWD/spirv_source/include', '-I$BASE_PWD/local_include', '-I$BASE_PWD/local_include/libdrm', '-march=armv7-a', '-mfloat-abi=softfp', '-mfpu=neon']\nc_link_args = ['--sysroot=$SYSROOT_PATH', '-Wl,--whole-archive', '-L$BASE_PWD/spirv_source/build_32/source/opt', '-lSPIRV-Tools-opt', '-L$BASE_PWD/spirv_source/build_32/source', '-lSPIRV-Tools', '-Wl,--no-whole-archive', '-lc', '-llog', '-landroid', '-ldl']\ncpp_link_args = ['--sysroot=$SYSROOT_PATH', '-Wl,--whole-archive', '-L$BASE_PWD/spirv_source/build_32/source/opt', '-lSPIRV-Tools-opt', '-L$BASE_PWD/spirv_source/build_32/source', '-lSPIRV-Tools', '-Wl,--no-whole-archive', '-lc', '-llog', '-landroid', '-ldl']\n[host_machine]\nsystem = 'linux'\ncpu_family = 'arm'\ncpu = 'armv7-a'\nendian = 'little'\n" > cross32.txt
