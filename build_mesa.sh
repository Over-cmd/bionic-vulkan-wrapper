...
# EL SECRETO DEL PESO REAL: Redefinimos los archivos .pc de SPIRV para que hereden el peso estático
printf "Name: SPIRV-Tools\nDescription: SPIRV Tools\nVersion: 2024.1\nLibs: -L%s/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/arm-linux-androideabi/26 -lSPIRV-Tools\nCflags:\n" "$NDK_PATH" > local_pkgconfig/SPIRV-Tools.pc
printf "Name: SPIRV-Tools-opt\nDescription: SPIRV Tools Opt\nVersion: 2024.1\nLibs: -L%s/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/arm-linux-androideabi/26 -lSPIRV-Tools-opt\nCflags:\n" "$NDK_PATH" > local_pkgconfig/SPIRV-Tools-opt.pc
...
# BYPASS DE LIBDL: Inyectamos la propiedad [constants] y propiedades de anulación para que Meson use libc en lugar de buscar la libdl.so suelta
...
# El truco maestro de Pipetto para inyectar un archivo de mapeo interno que anule la busqueda 'dl' en caliente
mkdir -p meson_links
printf "Name: dl\nDescription: Linker override\nVersion: 1.0\nLibs: -lc\n" > local_pkgconfig/dl.pc
...
# Redefinimos los archivos .pc de SPIRV para la Etapa 2 de 64 bits
printf "Name: SPIRV-Tools\nDescription: SPIRV Tools\nVersion: 2024.1\nLibs: -L%s/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/aarch64-linux-android/26 -lSPIRV-Tools\nCflags:\n" "$NDK_PATH" > local_pkgconfig/SPIRV-Tools.pc
printf "Name: SPIRV-Tools-opt\nDescription: SPIRV Tools Opt\nVersion: 2024.1\nLibs: -L%s/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/aarch64-linux-android/26 -lSPIRV-Tools-opt\nCflags:\n" "$NDK_PATH" > local_pkgconfig/SPIRV-Tools-opt.pc
