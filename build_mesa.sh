...
# TRUCO DE PESO BRUTO: Reemplazamos 'llvm-strip' por '/bin/true' para que el compilador no borre los símbolos ni achique el archivo final
...
meson setup build32 --cross-file arm32_cross.txt --buildtype=release -Dbuildtype=release -Doptimization=3 -Dplatforms=android -Dplatform-sdk-version=26 -Dvulkan-drivers=wrapper -Dgallium-drivers= --wrap-mode=nodownload
ninja -C build32

echo "=== 3. Configurando ETAPA 2: 64 BITS ==="
# Aplicamos la misma desactivación de strip en la Etapa 2 de 64 bits para retener los megabytes reales
...
meson setup build64 --cross-file arm64_cross.txt --buildtype=release -Dbuildtype=release -Doptimization=3 -Dplatforms=android -Dplatform-sdk-version=26 -Dvulkan-drivers=wrapper -Dgallium-drivers= --wrap-mode=nodownload
ninja -C build64
...
echo "Verificando el tamaño bruto real y pesado del driver:"
...
echo "Empaquetado masivo completado con éxito."
