...
echo "=== 3. Inyectando cabecera xf86drm.h definitiva ==="
...
# MODIFICACIÓN MAESTRA DEFINITIVA: Definimos VkXlibSurfaceCreateInfoKHR y VkXcbSurfaceCreateInfoKHR como estructuras de C reales asignables de tamaño fijo (struct { int dummy; }) para que no de error la asignación directa *out = *in;
printf "[binaries]\nc = '%s/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi26-clang'\ncpp = '%s/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi25-clang++'\nar = '%s/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-ar'\nstrip = '%s/toolchains/llvm/prebuilt/linux-x86_64/bin/llvm-strip'\npkg-config = 'pkg-config'\nllvm-config = '/usr/bin/llvm-config'\n[built-in options]\nc_args = ['-I%s/spirv_source/include', '-I%s/local_include', '-I%s/local_include/libdrm', '-I%s/local_include/bits', '-DVkXlibSurfaceCreateInfoKHR=struct{int dummy;}', '-DVkXcbSurfaceCreateInfoKHR=struct{int dummy;}']\ncpp_args = ['-I%s/spirv_source/include', '-I%s/local_include', '-I%s/local_include/libdrm', '-I%s/local_include/bits', '-DVkXlibSurfaceCreateInfoKHR=struct{int dummy;}', '-DVkXcbSurfaceCreateInfoKHR=struct{int dummy;}']\n[properties]\nlib_dirs = ['%s']\n[host_machine]\nsystem = 'android'\ncpu_family = 'arm'\ncpu = 'armv7-a'\nendian = 'little'\n" "$NDK_PATH" "$NDK_PATH" "$NDK_PATH" "$NDK_PATH" "$BASE_PWD" "$BASE_PWD" "$BASE_PWD" "$BASE_PWD" "$BASE_PWD" "$BASE_PWD" "$BASE_PWD" "$BASE_PWD" "$CLANG_LIB_DIR" > arm32_cross.txt
