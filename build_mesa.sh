...
echo "=== 1. Parcheando en caliente wsi_common_ahardware_buffer.c ==="
# El Truco Maestro: Modificamos el validador de buffers de Android para que retorne
# extension no presente de forma directa en lugar de buscar miembros inexistentes en la estructura.
if [ -f "src/vulkan/wsi/wsi_common_ahardware_buffer.c" ]; then
  printf "#include \"wsi_common.h\"\n#include <vulkan/vulkan.h>\n#include <vulkan/vulkan_android.h>\nVkResult wsi_common_vkGetAndroidHardwareBufferPropertiesANDROID(const VkDevice device, const struct AHardwareBuffer *buffer, VkAndroidHardwareBufferPropertiesANDROID *pProperties) { return VK_ERROR_EXTENSION_NOT_PRESENT; }\n" > src/vulkan/wsi/wsi_common_ahardware_buffer.c
fi

echo "=== 2. Creando cabecera de compatibilidad bits/pthreadtypes.h ==="
touch local_include/bits/pthreadtypes.h

echo "=== 3. Inyectando cabecera xf86drm.h definitiva con tipado de dispositivos ==="
