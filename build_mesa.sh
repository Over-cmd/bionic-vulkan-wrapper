...
# SOPLETE DE ENTORNO ATÓMICO: Inyectamos las 3 variables que exigen las estructuras originales de Android WSI en Mesa 24 de fábrica
...
# INYECCIÓN DIRECTA EN MESON: Pasamos los modificadores obligatorios de plataforma dentro de las opciones de enlace de Meson para sellar el puente gráfico nativo
meson setup build64 --cross-file cross64.txt --buildtype=release -Doptimization=2 -Dwerror=false -Dplatforms=android -Dplatform-sdk-version=26 -Dandroid-strict=false -Dvulkan-drivers=wrapper -Dgallium-drivers=[] -Dshared-glapi=enabled -Dllvm=disabled -Dvideo-codecs=[] -Db_rpath=false --wrap-mode=nodownload -Dc_args="-DANDROID=1 -DVK_USE_PLATFORM_ANDROID_KHR=1 -DHAVE_ANDROID_PLATFORM=1" -Dcpp_args="-DANDROID=1 -DVK_USE_PLATFORM_ANDROID_KHR=1 -DHAVE_ANDROID_PLATFORM=1" -Dc_link_args="-L$NDK_LIB_DIR_64 -L$SYSROOT_PATH/usr/lib/aarch64-linux-android/26 -lc -llog -landroid -ldl -lglslang -lclc" -Dcpp_link_args="-L$NDK_LIB_DIR_64 -L$SYSROOT_PATH/usr/lib/aarch64-linux-android/26 -lc -llog -landroid -ldl -lglslang -lclc"
...
meson setup build32 --cross-file cross32.txt --buildtype=release -Doptimization=2 -Dwerror=false -Dplatforms=android -Dplatform-sdk-version=26 -Dandroid-strict=false -Dvulkan-drivers=wrapper -Dgallium-drivers=[] -Dshared-glapi=enabled -Dllvm=disabled -Dvideo-codecs=[] -Db_rpath=false --wrap-mode=nodownload -Dc_args="-DANDROID=1 -DVK_USE_PLATFORM_ANDROID_KHR=1 -DHAVE_ANDROID_PLATFORM=1" -Dcpp_args="-DANDROID=1 -DVK_USE_PLATFORM_ANDROID_KHR=1 -DHAVE_ANDROID_PLATFORM=1" -Dc_link_args="-L$NDK_LIB_DIR_32 -L$SYSROOT_PATH/usr/lib/arm-linux-androideabi/26" -Dcpp_link_args="-L$NDK_LIB_DIR_32 -L$SYSROOT_PATH/usr/lib/arm-linux-androideabi/26"
...
echo "¡Tu Fat Binary unificado real ya compacto para Winlator Steven MXZ ha sido forjado con éxito total!"
