...
# EL ANCLAJE IMÁN DEFINITIVO 64 BITS: Reemplazamos la cadena única de SPIRV-Tools soldando el archivo físico .a de Qualcomm al vuelo de forma indestructible
sed -i 's|-lSPIRV-Tools|-lSPIRV-Tools '"$NDK_LIB_DIR_64"'/libadrenotools.a|g' build64/build.ninja
ninja -C build64
...
# EL ANCLAJE IMÁN DEFINITIVO 32 BITS: Lo mismo para el carril hermano de WoWBox64 utilizando su objeto monolítico correspondiente
sed -i 's|-lSPIRV-Tools|-lSPIRV-Tools '"$NDK_LIB_DIR_32"'/libadrenotools.a|g' build32/build.ninja
ninja -C build32
