...
if [ ! -d "libclc_source" ]; then
  ...
  # Jalamos únicamente la subcarpeta libclc de la factoría LLVM para ahorrar espacio
...
cmake .. -G Ninja -DCMAKE_TOOLCHAIN_FILE="$NDK_PATH/build/cmake/android.toolchain.cmake" -DANDROID_ABI=arm64-v8a -DANDROID_PLATFORM=android-26 -DCMAKE_BUILD_TYPE=Release -DLIBCLC_TARGETS_TO_BUILD="amdgcn--;nvptx--;r600--"
ninja -j $NPROC_CORES && cd ../../../..
...
cmake .. -G Ninja -DCMAKE_TOOLCHAIN_FILE="$NDK_PATH/build/cmake/android.toolchain.cmake" -DANDROID_ABI=armeabi-v7a -DANDROID_PLATFORM=android-26 -DCMAKE_BUILD_TYPE=Release -DLIBCLC_TARGETS_TO_BUILD="amdgcn--;nvptx--;r600--"
ninja -j $NPROC_CORES && cd ../../../..
...
# Sellar archivos compilados de libclc en las carpetas que Meson escaneará por defecto
