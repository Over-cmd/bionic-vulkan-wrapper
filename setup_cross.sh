...
# 1. RESTAURACIÓN DEL INTERCEPTOR PKG-CONFIG (Soluciona el error de libdrm actual)
echo -e '#!/bin/bash\nif [[ "$*" == *"--modversion"* ]]; then echo "14.0.0"; else echo "-I/tmp"; fi\nexit 0' > /tmp/fake-pkg-config
chmod +x /tmp/fake-pkg-config

# 2. ESCRITURA VERTICAL DEL ARCHIVO CRUZADO DE PIPETTO CON ARGS FLOTANTES
# Inyectamos -Wl,--allow-shlib-undefined de forma estricta. Esto le prohíbe al enlazador colapsar 
# por los archivos de PC del sysroot, forzando el cierre limpio del Fat Binary híbrido.
