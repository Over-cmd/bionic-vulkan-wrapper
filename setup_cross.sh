...
# RECETA DE ENLAZADO HÍBRIDO DE PIPETTO CON ZLIB NATIVA (Bypass definitivo de la línea 1582):
# Forzamos la bandera -lz de forma explícita en los argumentos de enlace para que Meson use la 
# librería real del sistema Android y deje de exigir la descarga o caída del subproyecto zlib.
...
c_link_args=['-llog', '-landroid', '-ldl', '-lz', '-Wl,--export-dynamic', '-Wl,--no-as-needed', '-L${LIB_64}']
cpp_link_args=['-llog', '-landroid', '-ldl', '-lz', '-Wl,--export-dynamic', '-Wl,--no-as-needed', '-L${LIB_64}', '-stdlib=libc++']
[host_machine]
system='linux'
cpu_family='aarch64'
cpu='armv8-a'
