...
# InterceptorPkg-config limpio
echo -e '#!/bin/bash\nif [[ "$*" == *"--modversion"* ]]; then echo "14.0.0"; else echo "-I/tmp"; fi\nexit 0' > /tmp/fake-pkg-config
chmod +x /tmp/fake-pkg-config

# LA RECETA MAESTRA DE PIPETTO:
# Configuramos el cross-file de 64 bits pero inyectamos macros de compatibilidad de 32 bits 
# (-D__ANDROID_VNDK__ y flags de emulación de punteros de 32 bits) para obligar a Clang a crear 
# un Fat Binary unificado que Winlator Steven-mxz leerá de golpe en ambos mundos.
