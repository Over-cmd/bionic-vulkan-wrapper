...
echo "=== 3. El Truco de Pipetto: Inyectando estructuras reales X11/Xcb en los encabezados del wrapper ==="
...
# Las inyectamos justo debajo del primer include en vk_printers.h y vk_unwrappers.h para que C y C++ las compartan en limpio
if [ -f "src/vulkan/wrapper/vk_printers.h" ]; then
  sed -i "s|#include <vulkan/vulkan.h>|#include <vulkan/vulkan.h>\n${ESTRUCTURAS_PC}|g" src/vulkan/wrapper/vk_printers.h
fi
